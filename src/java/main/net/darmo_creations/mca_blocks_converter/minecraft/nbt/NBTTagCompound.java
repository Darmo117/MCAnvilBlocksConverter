...
public class NBTTagCompound extends NBTBase {
  ...
  /**
   * Stores a new NBTTagLongArray with the given array as data into the map with the given string
   * key.
   */
  ...
  public byte[] getByteArray(String key) {
    try {
      return !hasKey(key, BYTE_ARRAY) ? new byte[0] : ((NBTTagByteArray) this.tagMap.get(key)).getByteArray();
    }
    catch (ClassCastException ex) {
      throw new RuntimeException(this.createCrashReport(key, BYTE_ARRAY, ex), ex);
    }
  }
  ...
  public int[] getIntArray(String key) {
    try {
      return !hasKey(key, INT_ARRAY) ? new int[0] : ((NBTTagIntArray) this.tagMap.get(key)).getIntArray();
    }
    catch (ClassCastException ex) {
      throw new RuntimeException(this.createCrashReport(key, INT_ARRAY, ex), ex);
    }
  }

  /**
   * Retrieves a long array using the specified key, or a zero-length array if no such key was
   * stored.
   */
  public long[] getLongArray(String key) {
    try {
      return !hasKey(key, LONG_ARRAY) ? new long[0] : ((NBTTagLongArray) this.tagMap.get(key)).getLongArray();
    }
    catch (ClassCastException ex) {
      throw new RuntimeException(this.createCrashReport(key, LONG_ARRAY, ex), ex);
    }
  }
