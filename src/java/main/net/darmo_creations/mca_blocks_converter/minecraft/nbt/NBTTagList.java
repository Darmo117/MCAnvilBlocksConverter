...
public class NBTTagList extends NBTBase {
  ...
  public int[] getIntArrayAt(int i) {
    if (i >= 0 && i < this.tagList.size()) {
      NBTBase nbtbase = this.tagList.get(i);
      return nbtbase.getId() == INT_ARRAY ? ((NBTTagIntArray) nbtbase).getIntArray() : new int[0];
    }
    else {
      return new int[0];
    }
  ...
  public long[] getLongArrayAt(int i) {
    if (i >= 0 && i < this.tagList.size()) {
      NBTBase nbtbase = this.tagList.get(i);
      return nbtbase.getId() == LONG_ARRAY ? ((NBTTagLongArray) nbtbase).getLongArray() : new long[0];
    }
    else {
      return new long[0];
    }
